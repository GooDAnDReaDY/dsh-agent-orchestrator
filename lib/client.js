...
`
    ...
    function KanbanTab({ config = {}, onConfigChange }) {
      ...
      return h('div', { className: 'dso-panel' },
        h('span', { className: 'dso-sub' },
          'Bidirectional synchronization with @goodandready/dsh-kanban: update task checklists and progress cards across execution stages.'
        ),
        h('div', { className: 'dso-agent-row' },
          ...
          h('div', { className: 'dso-grid-2', style: { marginTop: 10 } },
            h('div', { className: 'dso-field' },
              ...
              h('input', {
                className: 'dso-input',
                value: kanbanSync.targetColumn || 'Done',
                onChange: (e) => updateSync({ targetColumn: e.target.value }),
                placeholder: 'e.g. Done, Completed, Review',
              })
            ),
            h('div', { className: 'dso-field' },
    ...
    module.exports.apply = function apply(ctx) {
      ...
      try {
        ...
        console.warn('[dsh-agent-orchestrator] settings.plugin.item registration error:', err)
      }
      ...
      try {
        ...
        console.warn('[dsh-agent-orchestrator] settings.section registration error:', err)
      }
      ...
      try {
        ...
        console.warn('[dsh-agent-orchestrator] header widget registration error:', err)
      }
    }
