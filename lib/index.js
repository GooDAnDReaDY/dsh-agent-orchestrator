...
export const inject = ['settings', 'webServer', 'llm', 'tools', 'commands', 'systemPrompt']
  ...
  const runner = {
    async startPipeline({ taskTitle, taskDescription, scenarioId, taskId }) {
      ...
      const executor = async (stage, context) => {
        ...
        const role = cfg.roles.find((r) => r.id === stage.roleId) || {
          id: stage.roleId,
      ...
      executeDAG(plan.stages, executor, { maxConcurrency: 3 })
  ...
  ctx.inject(['tools'], (tctx) => {
    ...
    tctx.tools.define(
      defineTool({
        ...
        description:
          'Delegate a focused task directly to a specialized autonomous subagent (e.g. ui_design, architecture, spec, frontend, backend, qa_tests, code_review, security_audit, devops, docs, etc.) without running a full multi-stage DAG pipeline. Returns the specialist deliverable directly into this turn.',
        parameters: {
          roleId: {
            type: 'string',
            required: true,
        ...
        async execute(args) {
          ...
          return (
            `### 🧑‍💻 Specialist Deliverable: ${result.roleName} (\`${result.roleId}\`)\n` +
            `*Model: \`${result.model}\` | Prompt Cache Hit: ${hitPct}% (Estimated cost savings: ~${savingsPct}%)*\n\n` +
            `${result.output}`
          )
        },
      })
    )
  }, 'dsh-agent-orchestrator: register tools')
