...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PKG = '@goodandready/dsh-agent-orchestrator'
    const ROW_ID = 'dsh-agent-orchestrator'
...
`
    ...
    function OrchestratorSettingsCard(props) {
      ...
      // #139: the row seat (plugins.row.config) asks for two views. The summary is a
      // one-liner; the page renders the form expanded and without our card chrome,
      // because the host page draws the title, icon, crumb and padding itself.
      if (props && props.view === 'summary') {
        return h('div', { className: 'dso-sub' },
          'Decomposes tasks into 12 specialized roles with DAG execution.')
      }

      return h(page ? 'div' : 'li', { className: page ? 'dso-page' : 'dso-card' },
        h('button', {
          type: 'button',
          className: 'dso-head',
