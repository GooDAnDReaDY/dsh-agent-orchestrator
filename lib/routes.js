...
import { validateScenario } from './pipeline/scenarios.js'
...
export function registerOrchestratorRoutes(ctx, services) {
    ...
    handler: async (req, res) => {
      if (req.method === 'POST') {
        ...
        if (body?.scenarios && typeof body.scenarios === 'object') {
          for (const [sId, sc] of Object.entries(body.scenarios)) {
            try {
              validateScenario(sc)
            } catch (vErr) {
              return jsonReply(res, { error: `Invalid scenario "${sId}": ${vErr.message}` }, 400)
            }
          }
        }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (typeof snapshotId !== 'string' || !/^[a-zA-Z0-9_-]{1,64}$/.test(snapshotId)) {
        return jsonReply(res, { error: 'Invalid or unsafe snapshotId' }, 400)
      }

      try {
        const preset = snapshotManager.saveAsPreset(snapshotId, { name, description })
        validateScenario(preset)
