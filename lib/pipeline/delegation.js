 ...
 * - Per-Parent Serialization Gate & Concurrency Cap (Issue #93).
 * - Per-Call `cwd` Workspace Scoping & Sandbox containment check (Issue #87).
 * - Multi-layered Decision Trace Ledger in `presentationMeta` (≤4KB limit) (Issue #108).
 * - Session Lifecycle Auto-Archiving for one-shot subagents (Issue #56).
 *
 * @param {object} params
...
export async function executeDirectDelegation({
  ...
  try {
    ...
    // 4. Model resolution with allowedRoutes constraint
    ...
    const produceDeliverable = (data) => {
      ...
      const decisionTrace = createDecisionTrace({
        ...
        selectionReason: warning ? 'fallback_allowed' : 'preset_match',
        toolSummary: {
    ...
    // 5. Check for Native DSH subagent service (Issues #3, #110, #56)
    if (subagents && typeof subagents.getProvider === 'function') {
      ...
      if (transport) {
        ...
        try {
          ...
          // Mark session completed for auto-archiving grace-period (Issue #56)
          if (sessionLifecycle && run?.id) {
    ...
    // 6. Fallback: Direct LLM Turn with Prompt Cache Alignment (when native subagents service is absent)
