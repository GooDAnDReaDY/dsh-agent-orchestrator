  ...
  const updateConfig = async (partial) => {
    ...
    if (settingsScope) {
      try {
        ...
        console.debug('[dsh-agent-orchestrator] Settings update warning:', err?.message || err)
      }
    }
  }
  ...
  ctx.effect(() => () => {
    ...
    for (const ctrl of activeControllers) {
      try {
        ...
        console.debug('[dsh-agent-orchestrator] Controller abort error:', err?.message || err)
      }
    }
      ...
      executeDAG({
        ...
        .then(async (dagResult) => {
          ...
          try {
            ...
            console.debug('[dsh-agent-orchestrator] Snapshot record skipped:', snapErr?.message || snapErr)
          }
        })
        .catch((dagErr) => {
          store.updatePipeline(plan.pipelineId, {
  ...
  const registerToolsOnContext = (targetCtx) => {
    ...
    targetCtx.effect(() => {
      ...
      try {
        ...
        console.log('[dsh-agent-orchestrator] Registered tool: model_subagent_catalog')
      } catch (e) {
        console.error('[dsh-agent-orchestrator] Failed to register model_subagent_catalog:', e)
      }
