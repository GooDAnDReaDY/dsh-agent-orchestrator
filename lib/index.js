...
export const Config = Schema.object({
  enabled: Schema.boolean().default(true).description('Enable Multi-Agent Orchestrator pipeline dispatcher'),
  defaultScenario: Schema.string().default('auto').description('Default complexity scenario (auto, hotfix, simple, medium, complex, enterprise)'),
  cacheOptimization: Schema.object({
    enabled: Schema.boolean().default(true).description('Enable prefix canonicalization for prompt caching'),
    minTokens: Schema.number().default(1024).description('Minimum static anchor token count'),
  }),
  kanbanSync: Schema.object({
    enabled: Schema.boolean().default(true).description('Sync stage progress to @goodandready/dsh-kanban tasks'),
    autoAdvanceColumn: Schema.boolean().default(true).description('Automatically advance kanban task to review upon completion'),
  }),
})
    ...
    return ctx.tools.register(
      defineTool({
        ...
        parameters: {
          type: 'object',
          properties: {
            taskTitle: { type: 'string', description: 'Concise title of the objective' },
            taskDescription: { type: 'string', description: 'Detailed functional requirements and scope' },
            scenarioId: {
              type: 'string',
              enum: ['auto', 'hotfix', 'simple', 'medium', 'complex', 'enterprise'],
